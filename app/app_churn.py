...
from zoneinfo import ZoneInfo # Importa a biblioteca para fuso horário
...
def carregar_modelo():
    try:
        ...
        # Pega a data de modificação do arquivo do modelo (que é baseada em UTC)
        mod_time = os.path.getmtime(MODEL_PATH)
        
        # Converte a data do servidor para o fuso horário de São Paulo
        utc_datetime = datetime.fromtimestamp(mod_time, tz=ZoneInfo("UTC"))
        sao_paulo_datetime = utc_datetime.astimezone(ZoneInfo("America/Sao_Paulo"))
        
        # Formata a data para exibição
        model_date = sao_paulo_datetime.strftime('%d/%m/%Y às %H:%M:%S (Horário de Brasília)')
...
# --- O resto do código da Interface da Aplicação continua o mesmo ---
...
if model is not None:
    ...
    if st.button("Prever Churn", type="primary"):
        input_dict = {'gender': [gender], 'SeniorCitizen': [1 if senior_citizen == 'Yes' else 0],'Partner': [partner], 'Dependents': [dependents], 'tenure': [tenure],'PhoneService': [phone_service], 'PaperlessBilling': [paperless_billing],'MonthlyCharges': [monthly_charges], 'TotalCharges': [total_charges],'InternetService': [internet_service], 'Contract': [contract],'PaymentMethod': [payment_method]}
